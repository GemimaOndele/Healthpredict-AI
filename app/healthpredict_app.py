...
# OCR images & PDF (Tesseract + Poppler), Similarité historique, Traduction dataset FR, Exports CSV/Excel
import streamlit as st
import pandas as pd
import altair as alt
import matplotlib.pyplot as plt
import numpy as np
import io, json, joblib, yaml, os, warnings, unicodedata, re, datetime as dt
...
load_dotenv()
import scripts.download_assets  # pour s'assurer que le module est importé
try:
...
def load_cfg():
    ...
    return {
        "paths": {
            "processed_csv": "data/processed/medical_imaging_text_labeled.csv",  # ton ~33k
            "tfidf_model": "models/healthpredict_model.joblib",
            "camembert_model": "models/healthpredict_camembert_model.joblib",
            "raw_csv": "data/raw/openfda_maude.csv"  # si tu l'as via 01_download_openfda.py
        },
        "openfda": {
...
def reco(row):
    ...
    if alerte.startswith("Critique"):  return "⚠️ Maintenance immédiate requise"
    if alerte.startswith("Modérée"):   return "🔍 Surveillance accrue"
...
def translate_text(texts, src="en", tgt="fr", max_len=512, batch_size=16):
    ...
    tok, model, device = load_marian(src, tgt)
    out = []
...
def translate_to_fr_if_needed(raw_txt: str, lang_choice: str, force_to_fr: bool = True):
    ...
    if not raw_txt.strip(): return raw_txt, "fr", False
    if lang_choice == "Français": src = "fr"
    elif lang_choice == "Anglais": src = "en"
    else: src = detect_lang_simple(raw_txt)
...
def camembert_proba_fn(texts, tokenizer, camembert, clf):
    ...
    with torch.no_grad():
        ...
        # si ton clf attend la classe 1 en colonne 1 :
...
def add_detected_type(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df2["TypeDetecte"] = df2.apply(
        lambda r: detect_device_type_row(
            r.get(g, "") if g else "",
            r.get(b, "") if b else "",
            r.get(tcol, "")
        ), axis=1
...
def top_similar(text_query: str, pipe, X_hist, row_norms, df_hist: pd.DataFrame, text_col: str, k=10):
    if pipe is None or X_hist is None: return []
    vec = get_vectorizer_from_pipeline(pipe)
...
# Onglets
tab_dash, tab_pred, tab_docs, tab_hist = st.tabs(["📊 Tableau de bord", "🤖 Prédiction", "📎 Documents", "📚 Historique & Prévisions"])
...
with tab_dash:
    ...
    if len(df_view) > max_rows:
        # ✅ correction ici (len(df_view) au lieu de len[df_view])
    ...
    if "date_dt" in df_view.columns and df_view["date_dt"].notna().any():
        ...
        chart = alt.Chart(gr).mark_line(point=True).encode(
            x="mois:T", y="Incidents:Q"
        ).properties(height=280)
        st.altair_chart(chart, use_container_width=True)
    else:
        st.caption("Pas de dates exploitables pour la tendance.")

# Dans TAB Dashboard, après avoir défini df_view
if "event_type" in df_view.columns:
    sev_all = sorted([s for s in df_main["event_type"].dropna().unique()])
    ...
    al_all = sorted([s for s in df_main["Alerte"].dropna().unique()])
...
@st.cache_resource
def _pipe_tfidf(path): return load_tfidf_model(path)
@st.cache_resource
def _camembert(path): return load_camembert(path)
...
with tab_pred:
    ...
    if texte_alerte:
        ...
        if translated:
            ...
            with st.expander("Voir le texte traduit"): st.write(txt_for_model)
        ...
        if kw:
            ...
            st.download_button("⬇️ Exporter (CSV)", df_kw.to_csv(index=False).encode("utf-8"),
                               "mots_cles_prediction.csv", "text/csv", key="dl_kw_pred")
        else:
                ...
                if sims:
                    ...
                    for s, r in sims:
                        rows.append({
                            ...
                            "Extrait": (ensure_str(r.get(text_col_main,""))[:200] + "…")
                                      if len(ensure_str(r.get(text_col_main,""))) > 200 else ensure_str(r.get(text_col_main,""))
                        })
...
with tab_docs:
    ...
    tesseract_path = st.text_input("Chemin Tesseract (Windows)",
                                   value=r"C:\Program Files\Tesseract-OCR\tesseract.exe",
                                   key="inp_tess_path")
    poppler_path = st.text_input("Chemin Poppler (bin) pour PDF (Windows)",
                                 value=r"C:\Program Files\poppler-24.08.0\Library\bin",
                                 key="inp_poppler_path")
    ...
    if ups:
        ...
        for up in ups:
            with st.container(border=True):
                ...
                if use_pdf_ocr and name.endswith(".pdf") and not text.strip() and convert_from_bytes is not None:
                    ...
                    pdf_bytes = up.read()
                    if ready_tess:
                        text = extract_text_from_pdf_with_ocr(pdf_bytes, lang=ocr_langs,
                                                              poppler_path=poppler_path, dpi=pdf_dpi,
                                                              max_pages=max_pdf_ocr_pages) or ""
                    else:
                ...
                st.markdown(f"**Résultat :** {'🔴 Critique' if label=='Critique' else '🟢 Pas critique'} — **Confiance : {proba:.2f}** — **Type détecté : {type_doc}**")
        ...
        if all_kw_rows:
            ...
            st.download_button("⬇️ Exporter mots-clés (CSV)", df_all.to_csv(index=False).encode("utf-8"),
                               "mots_cles_documents.csv", "text/csv", key="dl_kw_docs_csv")
            buf_xlsx = io.BytesIO()
            with pd.ExcelWriter(buf_xlsx, engine="openpyxl") as writer:
                df_all.to_excel(writer, index=False, sheet_name="MotsCles")
            st.download_button("⬇️ Exporter mots-clés (Excel)", data=buf_xlsx.getvalue(),
...
with tab_hist:
    ...
    if "date_dt" in dft.columns and dft["date_dt"].notna().any():
        ...
        if len(series) >= 5:
            ...
            st.info(f"Taux observé λ ≈ **{lam:.3f}** inc./jour — écart moyen ≈ **{expected_gap:.1f} jours**.\n"
    ...
    if st.button("Lancer la traduction et exporter (Excel)", key="btn_trad_excel"):
        ...
        st.download_button("⬇️ Télécharger dataset traduit (Excel)", data=buf.getvalue(),
    ...
    with st.expander("📎 Sources & collecte"):
        of = cfg.get("openfda", {})
        st.write(f"**OpenFDA**: {of.get('base_url','?')} — période: {of.get('start_date','?')} → {of.get('end_date','?')} — "
                 f"mots-clés: {', '.join(of.get('keywords',[]))} — max: {of.get('max_records','?')}")
        st.caption("Collecte automatisée possible avec `scripts/01_download_openfda.py` (voir dépôt). Respect des licences (Kaggle, PhysioNet…).")
...
st.markdown("**Remerciements :** OpenAI pour l'API GPT-3.5-turbo, Hugging Face pour CamemBERT, et tous les contributeurs de la communauté.")
st.markdown("**Note :** Ce projet est à but éducatif et ne doit pas être utilisé pour des décisions médicales réelles sans validation par des experts.")
st.markdown("**Avertissement :** Les résultats de l'IA sont basés sur des données historiques et ne garantissent pas la précision des prédictions futures. Utilisez avec précaution.")
st.markdown("**Confidentialité :** Aucune donnée personnelle n'est collectée ou stockée par cette application. Toutes les données sont traitées localement et ne sont pas partagées avec des tiers.")
st.markdown("**Licence :** Ce projet est sous licence MIT. Consultez le fichier LICENSE pour plus de détails.")
st.markdown("**Contribuer :** Vous pouvez contribuer à ce projet en soumettant des issues ou des pull requests sur GitHub. Vos contributions sont les bienvenues !")
st.markdown("**Support :** Si vous avez des questions ou des suggestions, n'hésitez pas à me contacter par email ou sur LinkedIn. Je suis toujours ouvert aux retours et aux discussions sur l'amélioration de ce projet.")
st.markdown("**À propos :** HealthPredict AI est un projet visant à démontrer l'utilisation de l'IA pour la maintenance prédictive des équipements médicaux. Il s'agit d'une application de démonstration et ne doit pas être utilisée pour des décisions critiques sans validation appropriée.")
st.markdown("**Disclaimer :** Les résultats de l'IA sont fournis à titre informatif et ne doivent pas remplacer les conseils d'un professionnel de la santé ou d'un technicien qualifié. Utilisez les résultats avec discernement et en complément d'une expertise humaine.")
st.markdown("**Merci de votre intérêt pour HealthPredict AI !**")
st.markdown("**Pour en savoir plus :** Consultez la documentation du projet sur GitHub pour des guides d'utilisation, des exemples de données et des instructions de contribution. Votre participation est essentielle pour améliorer ce projet et le rendre plus utile pour la communauté.")   