# hpdb.py — SQLite helpers pour HealthPredict AI
...
DB_PATH = os.environ.get("HP_DB", str(DATA_DIR / "app.db"))
...
def insert_prediction(
    ...
    payload = {
        ...
        "top_keywords": json.dumps(top_keywords or [], ensure_ascii=False),
    }
...
def search_predictions(query: str, limit: int = 200, db_path: Optional[str] = None) -> List[Dict[str, Any]]:
    ...
    with _connect(db_path) as con:
        ...
        cur.execute("""
            SELECT * FROM predictions
            WHERE (file_name LIKE ? OR input_text LIKE ?)
            ORDER BY datetime(ts) DESC
            LIMIT ?
        """, (q, q, int(limit)))
        rows = cur.fetchall()
